  ...
  return [
    ...
    },
    {
      ...
      buildStart: function buildStart() {
        ...
        var hasHash =
          fileName == null || fileName.includes == null ? void 0 : fileName.includes('[hash');
        var emitFileOptions = {
          name: entryName,
          type: 'chunk',
          id: entryPath,
      ...
      generateBundle: function generateBundle(options, bundle) {
        ...
        var resolvePath = function resolvePath(htmlFileName) {
          ...
          if (
            !(
              (_viteConfig$experimen = viteConfig.experimental) != null &&
              _viteConfig$experimen.renderBuiltUrl
            )
          ) {
...
function checkAliasConflicts(options) {
  ...
  return {
    ...
    configResolved: function configResolved(config) {
      ...
      var userAliases =
        ((_config$resolve = config.resolve) == null ? void 0 : _config$resolve.alias) || [];
      var conflicts = [];
      for (var _i = 0, _sharedKeys = sharedKeys; _i < _sharedKeys.length; _i++) {
        var sharedKey = _sharedKeys[_i];
        for (
          var _iterator = _createForOfIteratorHelperLoose(userAliases), _step;
      ...
      estreeWalker.walk(ast, {
        enter: function enter(node) {
          if (node.type === 'ExportNamedDeclaration' && node.specifiers) {
            ...
            var proxyStatements = exportSpecifiers
              .map(function (name) {
                return (
                  '\n              const __mfproxy__await' +
                  name +
                  ' = await ' +
                  name +
                  '();\n              const __mfproxy__' +
              ...
              .join('\n');
            var exportStatements = exportSpecifiers
              .map(function (name) {
          ...
          if (node.type === 'ExportDefaultDeclaration') {
            ...
            if (declaration.type === 'Identifier') {
              ...
              proxyStatement =
                '\n                const __mfproxy__awaitdefault = await (' +
                code.slice(declaration.start, declaration.end) +
                ');\n                const __mfproxy__default = __mfproxy__awaitdefault;\n              ';
            }
            var _replacement =
...
var DevWorker = /*#__PURE__*/ (function () {
  ...
  _proto.update = function update() {
    ...
    (_this$worker$process = this.worker.process) == null ||
      _this$worker$process.send == null ||
      _this$worker$process.send({
...
var buildDtsModuleFederationConfig = function buildDtsModuleFederationConfig(options) {
  ...
  Object.entries(options.remotes).forEach(function (_ref2) {
    ...
    var entryLooksLikeUrl =
      ((_remote$entryGlobalNa = remote.entryGlobalName) == null
        ? void 0
        : _remote$entryGlobalNa.startsWith('http')) ||
      ((_remote$entryGlobalNa2 = remote.entryGlobalName) == null
        ? void 0
        : _remote$entryGlobalNa2.includes('.json'));
    var entryGlobalName = entryLooksLikeUrl
      ? remote.name || key
...
var normalizeDevDtsOptions = function normalizeDevDtsOptions(dts, context) {
  ...
  return sdk.normalizeOptions(
    dtsPlugin.isTSProject(dts, context),
    {
      generateTypes: defaultGenerateTypes,
      consumeTypes: defaultConsumeTypes,
      extraOptions: {},
      displayErrorInTerminal:
        typeof dts === 'object' && dts ? dts.displayErrorInTerminal : undefined,
    },
    'mfOptions.dts'
  )(dts);
};
...
function pluginDts(options) {
  ...
  var devPlugin = {
    ...
    configureServer: function configureServer(server) {
      ...
      var normalizedGenerateTypes = sdk.normalizeOptions(
      ...
      var normalizedConsumeTypes = sdk.normalizeOptions(
        Boolean(normalizedDtsOptions),
        {
          consumeAPITypes: true,
        },
      ...
      var startDevWorker = function startDevWorker() {
        try {
          var _temp2 = function _temp2() {
            ...
            (_server$httpServer = server.httpServer) == null ||
              _server$httpServer.once('close', function () {
                var _devWorker2;
  ...
  var buildPlugin = {
    ...
    generateBundle: function generateBundle() {
      try {
        ...
        try {
          normalizedDtsOptions = dtsPlugin.normalizeDtsOptions(
        ...
        var _temp5 = (function (_consumeOptions) {
          if (
            (_consumeOptions = consumeOptions) != null &&
            (_consumeOptions = _consumeOptions.host) != null &&
            _consumeOptions.typesOnBuild
          ) {
            var _temp3 = _catch(
              function () {
                return Promise.resolve(dtsPlugin.consumeTypesAPI(consumeOptions)).then(
                  function () {}
                );
              },
              function (error) {
...
function searchPackageVersion(sharedName) {
  try {
    ...
    while (
      path__namespace.parse(potentialPackageJsonDir).base !== 'node_modules' &&
...
function normalizeShareItem(key, shareItem) {
  ...
  try {
    try {
      version = require(
        path__namespace.join(removePathFromNpmPackage(key), 'package.json')
      ).version;
    } catch (e1) {
      try {
        var localPath = path__namespace.join(
          process.cwd(),
          'node_modules',
...
function assertModuleFound(tag, str) {
  ...
  if (!module) {
    throw new Error(
      "Module Federation shared module '" +
        str +
        "' not found. Please ensure it's installed as a dependency in your package.json."
    );
  }
  return module;
}
var VirtualModule = /*#__PURE__*/ (function () {
  ...
  VirtualModule.ensureVirtualPackageExists = function ensureVirtualPackageExists() {
    ...
    if (!fs.existsSync(virtualPackagePath)) {
      ...
      fs.writeFileSync(
        path.resolve(virtualPackagePath, 'package.json'),
        JSON.stringify({
  ...
  VirtualModule.findModule = function findModule(tag, str) {
    ...
    if (!patternMap[tag])
      patternMap[tag] = new RegExp(
        '(.*' + packageNameEncode(tag) + '(.+?)' + packageNameEncode(tag) + '.*)'
      );
  ...
  _proto.getImportId = function getImportId() {
    ...
    return (
      virtualModuleDir +
      '/' +
      packageNameEncode('' + mfName + this.tag + this.name + this.tag) +
      this.suffix
    );
  };
    ...
    Object.keys(options.exposes)
      .map(function (key) {
        return (
          ...
          JSON.stringify(options.exposes[key]['import']) +
...
function generateRemotes(id, command) {
  return (
    '\n    const {initPromise} = require("' +
    virtualRuntimeInitStatus.getImportId() +
    '")\n    const res = initPromise.then(runtime => runtime.loadRemote(' +
    JSON.stringify(id) +
    '))\n    const exportModule = ' +
    (command !== 'build' ? '/*mf top-level-await placeholder replacement mf*/' : 'await ') +
...
function writeLoadShareModule(pkg, shareItem, command) {
  loadShareCacheMap[pkg].writeSync(
    '\n    ;() => import(' +
      JSON.stringify(getPreBuildLibImportId(pkg)) +
      ...
      shareItem.shareConfig.singleton +
      ',\n        strictVersion: ' +
      shareItem.shareConfig.strictVersion +
      ',\n        requiredVersion: ' +
      JSON.stringify(shareItem.shareConfig.requiredVersion) +
      '\n      }}\n    }))\n    const exportModule = ' +
      (command !== 'build' ? '/*mf top-level-await placeholder replacement mf*/' : 'await ') +
  ...
  return (
    ...
    Array.from(getUsedShares())
      ...
      .map(function (pkg) {
        ...
        return (
          ...
          JSON.stringify(pkg) +
          ': async () => {\n          ' +
          ((shareItem == null ? void 0 : shareItem.shareConfig['import']) === false
            ? "throw new Error(`Shared module '${" +
              JSON.stringify(pkg) +
              "}' must be provided by host`);"
            : 'let pkg = await import("' +
              getPreBuildLibImportId(pkg) +
              '");\n            return pkg;') +
          '\n        }\n      '
        );
      })
    ...
    Array.from(getUsedShares())
      ...
      .map(function (key) {
        ...
        return (
          ...
          JSON.stringify(key) +
          ',\n            version: ' +
          JSON.stringify(shareItem.version) +
          ',\n            scope: [' +
          JSON.stringify(shareItem.scope) +
          '],\n            loaded: false,\n            from: ' +
    ...
    Object.keys(getUsedRemotesMap())
      .map(function (key) {
        ...
        return (
          '\n                {\n                  entryGlobalName: ' +
          JSON.stringify(remote.entryGlobalName) +
          ',\n                  name: ' +
          JSON.stringify(remote.name) +
          ',\n                  type: ' +
          JSON.stringify(remote.type) +
          ',\n                  entry: ' +
          JSON.stringify(remote.entry) +
          ',\n                  shareScope: ' +
          ((_JSON$stringify = JSON.stringify(remote.shareScope)) != null
            ? _JSON$stringify
            : 'default') +
          ',\n                }\n          '
        );
...
function generateRemoteEntry(options) {
  ...
  var importMapCode = Object.keys(options.shared || {})
    .sort()
    .map(function (pkg) {
  ...
  var usedSharedCode = Object.keys(options.shared || {})
    .sort()
    .map(function (key) {
    ...
    .join(',');
  var usedRemotesCode = Object.keys(getUsedRemotesMap())
    .map(function (key) {
      ...
      return (
        ...
        JSON.stringify(remote.entry) +
        ',\n        shareScope: ' +
        ((_JSON$stringify2 = JSON.stringify(remote.shareScope)) != null
          ? _JSON$stringify2
          : 'default') +
        ',\n      }'
      );
  ...
  return (
    ...
    options.shareScope +
    "', shared);\n    try {\n      await Promise.all(await initRes.initializeSharing('" +
    options.shareScope +
    "', {\n        strategy: '" +
...
var processModuleAssets = function processModuleAssets(bundle, filesMap, moduleMatcher) {
  ...
  for (
    var _i3 = 0, _Object$entries2 = Object.entries(bundle);
    ...
    for (
      var _i4 = 0, _Object$keys2 = Object.keys(fileData.modules);
      ...
      if (
        (_fileData$viteMetadat = fileData.viteMetadata) != null &&
        (_fileData$viteMetadat = _fileData$viteMetadat.importedCss) != null &&
        _fileData$viteMetadat.size
      ) {
        for (
          var _iterator = _createForOfIteratorHelperLoose(fileData.viteMetadata.importedCss), _step;
          !(_step = _iterator()).done;
      ...
      if (!foundCssViaMetadata && chunkContainsCssModules(fileData.modules)) {
        for (
          var _iterator2 = _createForOfIteratorHelperLoose(bundleCssAssets), _step2;
      ...
      if (fileData.dynamicImports) {
        for (
          var _iterator3 = _createForOfIteratorHelperLoose(fileData.dynamicImports), _step3;
...
var deduplicateAssets = function deduplicateAssets(filesMap) {
  var result = {};
  for (
    var _i5 = 0, _Object$entries3 = Object.entries(filesMap);
...
var buildFileToShareKeyMap = function buildFileToShareKeyMap(shareKeys, resolveFn) {
  try {
    ...
    return Promise.resolve(
      Promise.all(
        Array.from(shareKeys).map(function (shareKey) {
          return resolveFn(getPreBuildLibImportId(shareKey))
            .then(function (resolution) {
              var _resolution$id;
              return {
                shareKey: shareKey,
                file:
                  resolution == null || (_resolution$id = resolution.id) == null
                    ? void 0
                    : _resolution$id.split('?')[0],
              };
            })
            ['catch'](function () {
              return null;
      ...
      for (
        var _iterator4 = _createForOfIteratorHelperLoose(resolutions), _step4;
...
var Manifest = function Manifest() {
  ...
  if (typeof manifestOptions !== 'boolean') {
    mfManifestName = path__namespace.join(
      (manifestOptions == null ? void 0 : manifestOptions.filePath) || '',
      (manifestOptions == null ? void 0 : manifestOptions.fileName) || ''
    );
  }
  var root;
  var remoteEntryFile;
  ...
  return [
    {
      ...
      configureServer: function configureServer(server) {
        server.middlewares.use(function (req, res, next) {
          ...
          if (
            ((_req$url = req.url) == null ? void 0 : _req$url.replace(/\?.*/, '')) ===
            (viteConfig.base + mfManifestName).replace(/^\/?/, '/')
          ) {
            res.setHeader('Content-Type', 'application/json');
            res.setHeader('Access-Control-Allow-Origin', '*');
            res.end(
              JSON.stringify(
                _extends({}, generateMFManifest({}), {
                  id: name,
                  name: name,
                  metaData: {
                    name: name,
    ...
    },
    {
      ...
      generateBundle: function generateBundle(options, bundle) {
        try {
              ...
              var getPathWithoutKnownExt = function getPathWithoutKnownExt(filePath) {
                ...
                return JS_EXTENSIONS.includes(ext)
                  ? path__namespace.join(
                      ...
                      path__namespace.basename(filePath, ext)
                    )
                  : filePath;
              };
          ...
          return Promise.resolve(
            buildFileToShareKeyMap(getUsedShares(), _this.resolve.bind(_this))
          ).then(function (fileToShareKey) {
            processModuleAssets(bundle, filesMap, function (modulePath) {
  ...
  function generateMFManifest(preloadMap) {
    ...
    var shared = Array.from(getUsedShares())
      .map(function (shareKey) {
      ...
      .filter(Boolean);
    // Process exposed modules
    var exposes = Object.entries(options.exposes)
      .map(function (_ref3) {
        var key = _ref3[0],
          value = _ref3[1];
      ...
      var transformedCode = (function () {
        ...
        if (id.includes(getHostAutoInitPath())) {
          ...
          if (_command === 'serve') {
            ...
            var host =
              typeof ((_viteConfig$server = viteConfig.server) == null
                ? void 0
                : _viteConfig$server.host) === 'string' && viteConfig.server.host !== '0.0.0.0'
                ? viteConfig.server.host
                : 'localhost';
            var publicPath = JSON.stringify(
              resolvePublicPath(options, viteConfig.base) + options.filename
            );
            return (
              '\n          const origin = (window && ' +
              !options.ignoreOrigin +
              ') ? window.origin : "//' +
              host +
              ':' +
              ((_viteConfig$server2 = viteConfig.server) == null
                ? void 0
                : _viteConfig$server2.port) +
  ...
  return [
    ...
    },
    {
      ...
      config: function config(_config2, _ref) {
        ...
        var command = _ref.command;
        (_config2$resolve$alia = _config2.resolve.alias).push.apply(
          _config2$resolve$alia,
          Object.keys(shared).map(function (key) {
        ...
        (_config2$resolve$alia2 = _config2.resolve.alias).push.apply(
          _config2$resolve$alia2,
          Object.keys(shared).map(function (key) {
              ...
              : {
                  ...
                  customResolver: function customResolver(source, importer) {
                    try {
                      ...
                      var pkgName = module.name;
                      return Promise.resolve(
                        _this.resolve(pkgName, importer).then(function (item) {
                          return item.id;
                        })
                        ...
                        return Promise.resolve(savePrebuild.get(pkgName)).then(
                          function (_savePrebuild$get) {
                            return Promise.resolve(
                              _resolve.call(_this, _savePrebuild$get, importer)
                            );
                          }
                        );
                      });
...
var VarRemoteEntry = function VarRemoteEntry() {
  ...
  return [
    {
      ...
      configureServer: function configureServer(server) {
        server.middlewares.use(function (req, res, next) {
          ...
          if (
            ((_req$url = req.url) == null ? void 0 : _req$url.replace(/\?.*/, '')) ===
            (viteConfig.base + varFilename).replace(/^\/?/, '/')
          ) {
        ...
        try {
          ...
          if (!isValidName) {
            viteConfig.logger.warn(
              'Provided remote name "' +
                name +
                '" is not valid for "var" remoteEntry type, thus it\'s placed in globalThis[\'' +
                name +
          ...
          if (!remoteEntryFile)
            throw new Error(
              "Couldn't find a remoteEntry chunk file for " +
                mfOptions.filename +
                ", can't generate varRemoteEntry file"
            );
          _this.emitFile({
            type: 'asset',
  ...
  function generateVarRemoteEntry(remoteEntryFile) {
    ...
    return (
      ...
      '\n  ' +
      (isValidName ? name : "globalThis['" + name + "']") +
      ' = (function () {\n    function getScriptUrl() {\n      const currentScript = document.currentScript;\n      if (!currentScript) {\n        console.error("[VarRemoteEntry] ' +
      varFilename +
      " script should be called from sync <script> tag (document.currentScript is undefined)\")\n        return '/';\n      }\n      return document.currentScript.src.replace(/\\/[^/]*$/, '/');\n    }\n\n    const entry = getScriptUrl() + '" +
      remoteEntryFile +
...
function federation(mfUserOptions) {
  ...
  return [
    ...
    [pluginProxyRemoteEntry(), pluginProxyRemotes(options)],
      ...
      PluginDevProxyModuleTopLevelAwait(),
      {
        ...
        config: function config(_config, _ref) {
          var _config$optimizeDeps,
            _config$optimizeDeps2,
            _config$optimizeDeps3,
            _config$optimizeDeps4;
          // TODO: singleton
          _config.resolve.alias.push({
          ...
          (_config$optimizeDeps = _config.optimizeDeps) == null ||
            (_config$optimizeDeps = _config$optimizeDeps.include) == null ||
            _config$optimizeDeps.push('@module-federation/runtime');
          (_config$optimizeDeps2 = _config.optimizeDeps) == null ||
            (_config$optimizeDeps2 = _config$optimizeDeps2.include) == null ||
            _config$optimizeDeps2.push(virtualDir);
          (_config$optimizeDeps3 = _config.optimizeDeps) == null ||
            (_config$optimizeDeps3 = _config$optimizeDeps3.needsInterop) == null ||
            _config$optimizeDeps3.push(virtualDir);
          (_config$optimizeDeps4 = _config.optimizeDeps) == null ||
            (_config$optimizeDeps4 = _config$optimizeDeps4.needsInterop) == null ||
            _config$optimizeDeps4.push(getLocalSharedImportMapPath());
        },
      },
    ],
    Manifest(),
