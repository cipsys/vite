...
function generateRemoteEntry(options) {
  ...
  const importMapCode = Object.keys(options.shared || {}).sort().map(pkg => {
  ...
  const usedSharedCode = Object.keys(options.shared || {}).sort().map(key => {
  ...
  const usedRemotesCode = Object.keys(getUsedRemotesMap()).map(key => {
  ...
  return {
    ...
    async resolveId(id, importer) {
      ...
      if (importer === REMOTE_ENTRY_ID && !id.startsWith('.') && !id.startsWith('/') && !id.startsWith('\0') && !id.startsWith('virtual:')) {
        ...
        const resolved = await this.resolve(id, __filename, {
          skipSelf: true
        });
